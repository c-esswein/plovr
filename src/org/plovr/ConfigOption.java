...
public enum ConfigOption {
	...
	// The enum order is the order in which these options appear in the
	// generated
	...
	EXCLUDE_CLOSURE_LIBRARY("experimental-exclude-closure-library",
			new ConfigUpdater() {
				@Override
				public void apply(boolean excludeClosureLibrary,
	...
	WARNING_LEVEL("level", new ConfigUpdater() {
		...
		public void apply(String level, Config.Builder builder) {
			try {
				WarningLevel warningLevel = WarningLevel.valueOf(level
						.toUpperCase());
	...
	INHERITS("inherits", new ConfigUpdater() {
		@Override
		public void apply(String mode, Config.Builder builder) {
			// Do nothing: this option is handled in a special way in
			// ConfigParser.
	...
	PRINT_INPUT_DELIMITER("print-input-delimiter", new ConfigUpdater() {
		...
		@Override
		public boolean update(String printInputDelimiterParam,
				Config.Builder builder) {
			boolean printInputDelimiter = Boolean
					.valueOf(printInputDelimiterParam);
			builder.setPrintInputDelimiter(printInputDelimiter);
			return true;
		}
	}),
	...
	OUTPUT_FILE("output-file", new ConfigUpdater() {
		...
		public void apply(String outputFilePath, Config.Builder builder) {
			File outputFile = (outputFilePath == null) ? null : new File(
					maybeResolvePath(outputFilePath, builder));
	...
	MODULES("modules", new ConfigUpdater() {
		...
		public void apply(JsonObject modules, Config.Builder builder) {
			try {
				ModuleConfig.Builder moduleConfigBuilder = builder
	...
	// Note: even if sourcemaps are fixed, this option should still be
	// supported:
	// http://code.google.com/p/plovr/issues/detail?id=41
	// http://code.google.com/p/plovr/issues/detail?id=50
	MODULE_INFO_PATH("module-info-path", new ConfigUpdater() {
		@Override
	...
	DIAGNOSTIC_GROUPS("checks", new ConfigUpdater() {
		...
		public void apply(JsonObject obj, Config.Builder builder) {
			...
			for (Map.Entry<String, JsonElement> entry : obj.entrySet()) {
				String checkLevelString = GsonUtil.stringOrNull(entry
						.getValue());
				if (checkLevelString == null) {
					continue;
				}
				CheckLevel checkLevel = CheckLevel.valueOf(checkLevelString
						.toUpperCase());
	...
	NAME_SUFFIXES_TO_STRIP("name-suffixes-to-strip", new ConfigUpdater() {
		...
		public void apply(JsonArray suffixes, Config.Builder builder) {
			ImmutableSet.Builder<String> suffixesBuilder = ImmutableSet
					.builder();
	...
	ID_GENERATORS("id-generators", new ConfigUpdater() {
		...
		public void apply(JsonArray idGenerators, Config.Builder builder) {
			ImmutableSet.Builder<String> idGeneratorsBuilder = ImmutableSet
					.builder();
	...
	EXPERIMENTAL_COMPILER_OPTIONS("experimental-compiler-options",
			new ConfigUpdater() {
				@Override
	...
	CUSTOM_PASSES("custom-passes", new ConfigUpdater() {
		...
		public void apply(JsonArray value, Config.Builder builder) {
			ImmutableListMultimap.Builder<CustomPassExecutionTime, CompilerPassFactory> customPasses = ImmutableListMultimap
					.builder();
			Gson gson = new GsonBuilder().registerTypeAdapter(
					CustomPassConfig.class,
					new CustomPassConfig.CustomPassConfigDeserializer())
					.create();
			for (JsonElement entry : value) {
				CustomPassConfig pass = gson.fromJson(entry,
						CustomPassConfig.class);
				Class<?> clazz;
				try {
	...
	SOY_USE_INJECTED_DATA("soy-use-injected-data", new ConfigUpdater() {
		...
		@Override
		public boolean update(String soyUseInjectedDataParam,
				Config.Builder builder) {
			boolean soyUseInjectedData = Boolean
					.valueOf(soyUseInjectedDataParam);
			builder.setSoyUseInjectedData(soyUseInjectedData);
			return true;
		}
	}),
	...
	VARIABLE_MAP_INPUT_FILE("variable-map-input-file", new ConfigUpdater() {
		...
		public void apply(String file, Config.Builder builder) {
			File inputFile = (file == null) ? null : new File(maybeResolvePath(
					file, builder));
	...
	VARIABLE_MAP_OUTPUT_FILE("variable-map-output-file", new ConfigUpdater() {
		@Override
		public void apply(String file, Config.Builder builder) {
			File outputFile = (file == null) ? null : new File(
					maybeResolvePath(file, builder));
	...
	CSS_GSS_FUNCTION_MAP_PROVIDER("css-gss-function-map-provider",
			new ConfigUpdater() {
				@Override
				public void apply(String functionMapProviderClass,
						Config.Builder builder) {
	...
	LANGUAGES("languages", new ConfigUpdater() {
		@Override
		public void apply(JsonObject json, Config.Builder builder) {
			LanguageConfig langConfig = new LanguageConfig();

			for (Map.Entry<String, JsonElement> entry : json.entrySet()) {
				String filePath = entry.getValue().getAsString();
				File translationFile = new File(maybeResolvePath(filePath, builder));
				
				LanguageConfig.Language lang = new LanguageConfig.Language(
						entry.getKey(), translationFile);
				langConfig.addLanguage(lang);
			}

			builder.setLanguageConfig(langConfig);
		}
	})
	...
	private static class ConfigUpdater {
		...
		/**
		 * Only override this method if this option can be overridden using
		 * query data.
		 * 
		...
		public boolean update(String queryDataValue, Config.Builder builder) {
			// By default, does nothing. Only override if it is safe to update
			// the
	...
	static String maybeResolvePath(String path, File relativePathBase) {
		// Unfortunately, a File object must be constructed in order to
		// determine
