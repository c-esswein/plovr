...
public final class ModuleConfig {
	...
	private final Map<String, Function<String, File>> moduleToOutputPath;
	...
	private ModuleConfig(String rootModule,
			...
			Map<String, Function<String, File>> moduleToOutputPath, File moduleInfoPath,
	...
	public Map<String, File> getModuleToOutputPath() {
		Map<String, File> resolvedPaths = Maps.newHashMap();
		
		for(String moduleName : moduleToOutputPath.keySet()){
			File moduleOutputFile = moduleToOutputPath.get(moduleName).apply(language);
			resolvedPaths.put(moduleName, moduleOutputFile);
		}
	...
	List<JSModule> getModules(Manifest manifest) throws CompilationException {
		...
		// must either appear (i) before it in that module, or (ii) in an
		// ancestor
		// module. This means that if two sibling modules have the same
		// dependency,
		// then that dependency should appear in the least common ancestor
		// module
		...
		//
		// There are two reasonable options for how the inputs (and their
		// transitive
		// dependencies) should be alloted to the modules:
		//
		// Option 1: Put string.js in A even though the only file initially
		// assigned
		// to A (base.js) does not depend on string.js. Because both B and C
		// depend
		...
		// B C C contains asserts.js which depends on string.js
		//
		// The benefit of this solution is that the common dependencies of B and
		// C
		// no longer bloat A. If a sophisticated module loader is used on the
		// server and the client, it would be possible to request modules D and
		// B
		// in a single request, such that the server would be responsible for
		// concatenating the contents of both modules and returning them in a
		// single
		// response.
		//
		// For now, only Option 1 is implemented for simplicity. Because the
		// module
		// graph is a rooted DAG, the root module is built up first, followed by
		// its
		// children. As child modules are created, inputs (and their transitive
		// dependencies) may be promoted to ancestor modules to satisfy
		// dependency
		// constraints.
	...
	Map<String, List<JsInput>> partitionInputsIntoModules(Manifest manifest)
		...
		// When using the Closure Library, base.js is the first item on the list
		// and
		// should be removed.
		...
		Map<String, LinkedHashSet<JsInput>> moduleToTransitiveDependencies = Maps
				.newHashMap();
		Map<String, JsInput> provideToSource = manifest
				.getProvideToSource(ImmutableSet.copyOf(inputsInOrder));
		for (String module : topologicalSort) {
		...
		// which inputs have already been seen. For an input that is seen for
		// the
		// first time, find the set of modules that contain that input in its
		// set of
		// transitive dependencies. Once that set is created, find the least
		// common
		...
		for (JsInput input : inputsInOrder) {
			Set<String> modulesWithInput = findModulesWithInput(input,
					moduleToTransitiveDependencies);
			String ancestor = findLeastCommonAncestor(modulesWithInput,
					searcher);
			moduleToInputs.get(ancestor).add(input);
		}

		// Because it is a special case, add base.js as the first input in the
		// root
	 ...
	 */
	private static Set<String> findModulesWithInput(JsInput input,
			Map<String, LinkedHashSet<JsInput>> moduleToTransitiveDependencies) {
		Set<String> modules = Sets.newHashSet();
		for (Map.Entry<String, LinkedHashSet<JsInput>> entry : moduleToTransitiveDependencies
				.entrySet()) {
	...
	private String findLeastCommonAncestor(Set<String> modules,
			Searcher searcher) {
		Preconditions.checkArgument(modules.size() > 0);

		// If the set contains the root module, then the least common ancestor
		// of
		...
		// If the set has exactly one element, then it must be its own least
		// common
		// ancestor.
		...
		// modules in the set, in which case it is must be the least common
		// ancestor
		// for all of the modules in the set.
		// 2. A module that is not in the set is the least common ancestor for
		// all
		// of the modules.
		// It cannot be the case that no least common module exists because the
		// root
	...
	private class Searcher {
		...
		/**
		 * This is a map from Set of modules -> least common ancestor for these
		 * modules. Note that when inserting elements, the key must be an
		 * ImmutableSet, otherwise map invariants are not maintained.
		 */
		...
		/**
		 * This is a 2D array of minimum distances between modules along the
		 * DAG. The first index is the source module, and the second index is
		 * the ancestor module. The value is the minimum distance between the
		 * two. null means that there is no connection. The module -> integer
		 * index mapping is done by looking at moduleToIndex.
		 */
		...
		private Searcher() {
			...
			for (Map.Entry<String, Integer> module : moduleToIndex.entrySet()) {
				populateModuleDistances(module.getKey(), module.getValue(),
						module.getValue(), 0);
			}
		}
		...
		/**
		 * Populates the distance map by recursing down a module's dependencies.
		 * This function call should be initiated with identical moduleIndex and
		 * sourceModuleIndex, and will populate the distanceMap for that
		 * sourceModuleIndex.
		 */
		private void populateModuleDistances(String module, int moduleIndex,
				int sourceModuleIndex, int depth) {
			...
			for (String dep : moduleInfoMap.get(module).getDeps()) {
				populateModuleDistances(dep, moduleToIndex.get(dep),
						sourceModuleIndex, depth + 1);
			}
		}
	}
	...
	final static class Builder {
		...
		private Builder(ModuleConfig moduleConfig) {
			...
			// An outputPath set on this builder will be ignored as the
			// inherited
		...
		private static Map<String, List<String>> invertDependencyTree(
			...
			// dependencies maps a module to the modules that depend on it. A
			// module
			// must be loaded before any of its dependencies.
			Map<String, List<String>> dependencies = Maps.newHashMap();

			// Populate the keys of dependencies with the keys of the "deps"
			// object
			// literal from the config file.
			for (ModuleInfo info : moduleInfo) {
				dependencies
						.put(info.getName(), Lists.<String> newLinkedList());
			}
			...
			for (ModuleInfo info : moduleInfo) {
				...
				for (String ancestorModule : info.getDeps()) {
					...
					if (moduleDeps != null) {
						...
						// A dependent module must appear as a key in the "deps"
						// object
						// literal from the config file.
		...
		private static String findRootModule(Collection<ModuleInfo> dependencies)
			...
			if (moduleWithNoDependencies == null) {
				throw new BadDependencyTreeException(
						"There was no module with zero"
		...
		private static List<String> buildDependencies(
			...
			buildDependencies(dependencyTree, transitiveDependencies,
					rootModule, new LinkedHashSet<String>());
		 ...
		 *            the Iterator of this set returns the dependencies in
		 *            reverse order
		 * @param dependency
		 * @param currentChain
		 *            Tracks what the current path down the tree is
		 * @throws BadDependencyTreeException
		 */
		private static void buildDependencies(
				Map<String, List<String>> dependencies,
				LinkedHashSet<String> transitiveDependencies,
				String dependency, LinkedHashSet<String> currentChain)
				throws BadDependencyTreeException {
			if (currentChain.contains(dependency)) {
				throw new BadDependencyTreeException(
						"Circular module dependency: " + currentChain + " -> "
								+ dependency);
			}
			...
			for (String dependencyList : dependencies.get(dependency)) {
				buildDependencies(dependencies, transitiveDependencies,
						dependencyList, currentChain);
			}
		...
		public void setModuleInfo(JsonObject json)
			...
			for (Map.Entry<String, JsonElement> entry : json.entrySet()) {
				// Each JsonElement should be an object with "deps" and "inputs"
				// fields.
				...
				if (inputs == null) {
					throw new IllegalArgumentException(
							"Module "
									...
									+ " must specify inputs"
									+
									// TODO(bolinfest): Remove after this change
		...
		void setModuleInfo(Map<String, ModuleInfo> moduleInfo)
				throws BadDependencyTreeException {
			// In dependencyTree, modules point to the modules that depend on
			// them.
			...
			// Calling buildDependencies() confirms that the module dependencies
			// are
			...
			// }
			//
			// Then the iteration of transitiveDependencies would produce
			// either:
			//
			// ["A", "B", "C", "D", "E"] OR ["A", "B", "C", "E", "D"]
			//
			// Both are valid results because the topological sort of the
			// dependency
			// graph is not unique.
		...
		public ModuleConfig build() {
			Preconditions.checkState(dependencyTree != null,
					"No modules were set");

			// Set the paths to write the compiled module files to.
			Map<String, Function<String, File>> moduleToOutputPath;
			if (this.moduleToOutputPath == null) {
				moduleToOutputPath = Maps.newHashMap();
				for (final String moduleName : dependencyTree.keySet()) {
					Function<String, File> moduleUriFunction = new Function<String, File>() {
						@Override
						public File apply(String language) {
							String partialPath = populateModuleNameTemplate(
									outputPath, moduleName, language);
							File moduleFile = new File(
									ConfigOption.maybeResolvePath(partialPath,
											relativePathBase));
					...
					moduleToOutputPath.put(moduleName, moduleUriFunction);
				}
			} else {
			...
			if (this.moduleInfoPath == null) {
				if (infoPath == null) {
					...
					moduleInfoPath = new File(ConfigOption.maybeResolvePath(
							infoPath, relativePathBase));
				}
			} else {
			...
			return new ModuleConfig(rootModule, dependencyTree, moduleInfo,
					topologicalSort, moduleToOutputPath, moduleInfoPath,
					productionUri);
		}
	}
